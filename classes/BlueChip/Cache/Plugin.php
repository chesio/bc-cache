...
class Plugin
{
    ...
    private function startOutputBuffering(): void
    {
        if (!self::skipCache()) {
            if (is_wp_version_compatible('6.9')) {
                ...
                add_filter('wp_should_output_buffer_template_for_enhancement', '__return_true', 10, 0);
                // Save output buffer.
                add_action('wp_finalized_template_enhancement_output_buffer', $this->pushToCache(...), 10, 1);
            } else {
     ...
     *
     * @todo The handler can be removed when WordPress 6.9 or newer is required.
     */
    ...
    /**
     * Push $output to cache.
     */
    private function pushToCache(string $output): void
    {
        $item = new Item(
            Utils::getRequestUrl(),
